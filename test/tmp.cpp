...
void sift_test1B(
    char* path_data, //path to the dataset
	char* path_q, //path to the query set
    int vecsize_, // number of data points
	int vecdim_, // dimension of vectors
...
int QsortComp(const void* e1,const void* e2);	
void kmeans_(float** train,float** result,int n,int d);
void sub_kmeans_(
    float** train, //L * L * d
    float** result,  //L * d
...
void kmeans0_(float** train,float** result,int n,int d,int cen1);
void sub_kmeans0_(
    int ii, 
    float*** quantizer,   
    unsigned char* merge,
...
int main(void){
    ...
    char* path_data="test.ds";
    char* path_q="test.q";
    sift_test1B(path_data,path_q,10,10,1,3,0.5,-1);
    return 1;
}
...
void sift_test1B(
	...
	int efsearch){
        ...
        int dim_;
	    int max_num = pow(2, max_level_ + OFF); //to 4 by default, 16
        ...
        int in = 0;
	    //vecdim->vectordim  dim_->bottom layer dim
        L2Space l2space(vecdim, dim_);

		HierarchicalNSW<float> *appr_alg;

		//initilaization of hnsw index
		if(!exists_test(path_index)){
			cout<<"build up hnsw"<<endl;
    		appr_alg = new HierarchicalNSW<float>(level_, &l2space, vecsize, vecdim_, M, efConstruction);
		}
	
		int* length = new int[max_level_];// length on each level
		int* dim = new int[max_level_]; //dim on each level
		...
		for(int i=0;i<max_level_;i++){
			cout<<"length["<<i<<"]: "<<length[i]<<" dim["<<i<<"]: "<<dim[i]<<endl;
		}

		//loading index.bin if index.bin is exist, read in directly.
		if (exists_test(path_index)) {
	   		cout << "Loading queries:\n";
          	// float *massQ = new float[qsize * vecdim];
          	// ifstream inputQ(path_q, ios::binary);
           	// for (int i = 0; i < qsize; i++) {
            //    int in = 0; //dimentionality, in=vecdim;
		...
		//obj_quantizer[length[i]][vecsize][max_level_]
		...
		float**** quantizer = new float*** [max_level_];//quantizer[dim[i]][L][length[i]][max_level]
		float**** temp_quan = new float*** [max_level_];//quantizer[dim[i]][L*L][length[i]][max_level]
		...
		//train[min_dim][size_n(10000)][max_book]
		...
		//determine the train-org matrix
		//train_org[dim_][size_n]
    ...
    //add the first point at dataset
	...
	#pragma omp parallel for	
		...
		//appr_alg->addPoint((void *) (massb), (size_t) i, NULL, -1, true);
	...
	for(int ii = 0; ii <= max_level_-1; ii++){ 
	    ...
	    //cout<<"count2: "<<count2_<<" cont_obj[ii]: "<<count_obj[ii]<<endl;
